USE [new] /* USE [tst] */
	...
	UPDATE [tst].[dbo].[all_types_mssql_long]
	           set
	...
	UPDATE [tst].[dbo].[all_types_mssql] 
	           SET
	...
	set @i=0;
	SET @Lower = (select min(s_bigint) from [tst].[dbo].[all_types_mssql]); 
	SET @Upper = (select max(s_bigint) from [tst].[dbo].[all_types_mssql]); 
	
	while @i <5 
	
	begin
	delete from [tst].[dbo].[all_types_mssql_long]
	           where s_bigint = @Random;
	delete from [tst].[dbo].[all_types_mssql] 
	           where s_bigint = @Random;
