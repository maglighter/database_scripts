/****** Object:  Table [dbo].[all_types_mssql]    Script Date: 11/17/2011 13:38:35 ******/
	SET ANSI_NULLS ON
	GO---Задает совместимое со стандартом ISO поведение операторов сравнения «равно» (=) и «не равно» (<>) при использовании со значениями NULL.
	
	SET QUOTED_IDENTIFIER ON
	GO---Заставляет SQL Server следовать правилам ISO относительно разделения кавычками идентификаторов и строк-литералов.
	
	SET ANSI_PADDING ON
	GO---Контролирует способ хранения в столбце значений короче, чем определенный размер столбца, и способ хранения в столбце значений,
	--- имеющих замыкающие пробелы, в данных char, varchar, binary и varbinary.
	...
	CREATE TABLE [tst].[dbo].[all_types_mssql](
		...
		[s_time] [time](7) NULL,
		[s_real] [real] NULL,
	PRIMARY KEY CLUSTERED ---создавать кластеризованный индекс, т.е. такой индекс,
	--- при котором в листьях B-дерева, образующего индекс, находятся не ссылки на данные, а собственно страницы данных. 
	(
		[s_bigint] ASC
