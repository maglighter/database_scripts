...
select count(*) into v_cnt from EBF_chartypes;

DELETE FROM EBF_chartypes
...
DELETE FROM EBF_NUMBERTYPES
      WHERE s_pk_formapping =
                         (SELECT MAX (s_pk_formapping)
                            FROM (SELECT s_pk_formapping
                                    FROM EBF_NUMBERTYPES
                                   WHERE ROWNUM <= DBMS_RANDOM.VALUE (1, v_cnt)));
                                   

DELETE FROM EBF_RAWLOBTYPES
      WHERE s_pk_formapping =
                         (SELECT MAX (s_pk_formapping)
                            FROM (SELECT s_pk_formapping
                                    FROM EBF_RAWLOBTYPES
                                   WHERE ROWNUM <= DBMS_RANDOM.VALUE (1, v_cnt)));
commit;
...
for c in 1 .. 2 loop
   ...
   commit;
    ...
    -- updates
declare       
   cursor c is select * from EBF_chartypes 
                      where
                      s_pk_formapping =
                         (SELECT MAX (s_pk_formapping)
                            FROM (SELECT s_pk_formapping
...
declare   
   cursor c is select * from EBF_numbertypes 
                      where
                      s_pk_formapping =
                         (SELECT MAX (s_pk_formapping)
                            FROM (SELECT s_pk_formapping
...
declare
  cursor c is select * from EBF_datatypes 
                      where
                      s_pk_formapping =
                         (SELECT MAX (s_pk_formapping)
                            FROM (SELECT s_pk_formapping
...
declare
 cursor c is select * from EBF_rawlobtypes where s_pk_formapping = v_id for update;
